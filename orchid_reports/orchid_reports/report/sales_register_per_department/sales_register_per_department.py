...
def get_conditions(filters):
	...
	def get_sales_invoice_item_field_condition_parent_item_group(field, table="Sales Invoice Item") -> str:
		if not filters.get(field) or field in accounting_dimensions_list:
			return ""
		return f""" and exists(select name from `tab{table}`
				where parent=`tabSales Invoice`.name
					and ifnull(`tab{table}`.parent_item_group, '') = %({field})s)"""
	...
	if not user_permition:
		conditions += get_sales_invoice_item_field_condition_parent_item_group("item_group")
...
def get_invoices(filters, additional_query_columns):
	...
	if not user_permition:
		...
		return frappe.db.sql(
			...
			customer_name, owner, remarks, territory, tax_id, customer_group,
			base_net_total, base_grand_total, base_rounded_total, outstanding_amount,
			is_internal_customer, represents_company, company {additional_query_columns}
			from `tabSales Invoice`
			where docstatus = 1 and exists(select name from `tabSales Invoice Item`
			where parent=`tabSales Invoice`.name and ifnull(`tabSales Invoice Item`.parent_item_group, '') = '{user_permition}')  %s order by posting_date desc, name desc"""
			% conditions,
			filters,
			as_dict=1,
		)
