...
class Analytics(object):
	...
	def get_sales_transactions_based_on_customers_or_suppliers(self):
		...
		if user_permition:
			filters_value.append([self.filters.doc_type + " Item", "parent_item_group", "=", user_permition],)
		self.fainal_data = frappe.get_all(
		...
		self.entity_names = {}
		self.entries = []
		self.entries.append(self.fainal_data[0])
		for i in range(1,len(self.fainal_data)): 
			item_name =next((item for item in self.entries if item.name == self.fainal_data[i].name), "x")
			if item_name !='x':
				pass
			else:
				self.entries.append(self.fainal_data[i])
	...
	def get_sales_transactions_based_on_item_group(self):
		if self.filters["value_quantity"] == "Value":
			...
			value_field = "qty"
		# user_permition = frappe.db.get_value('User Permission',{'user': frappe.session.user,'allow':'Item Group'},'for_value')
		# user_permition_value = ""
		# if user_permition:
		# 	user_permition_value = f"and i.parent_item_group = '{user_permition}'"
		self.entries = frappe.db.sql(
			"""
			select i.item_group as entity, i.{value_field} as value_field, s.{date_field}
			from `tab{doctype} Item` i , `tab{doctype}` s
			where s.name = i.parent and i.docstatus = 1 and s.company = %s
			and s.{date_field} between %s and %s {user_permition_value}
		""".format(
				date_field=self.date_field, value_field=value_field, doctype=self.filters.doc_type,user_permition_value=user_permition_value
			),
			(self.filters.company, self.filters.from_date, self.filters.to_date),
			as_dict=1,
		)
